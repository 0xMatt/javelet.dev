...
export default function Contributions({data}: ContributionProps) {
    ...
    return (
        <div className='grid grid-cols-2 gap-3 py-2 sm:grid-cols-4'>
            <StatCard label='Total' icon={Activity} number={totalContributions}/>
            <StatCard label='This Week' icon={Activity} number={totalThisWeekContribution}/>
            <StatCard label='Best Day' icon={Activity} number={bestContribution}/>
            <StatCard label='Monthly Average' icon={Activity} number={averageContribution} unit='/month'/>
        </div>
    );
};
