...
const useAutoResizeTextarea = ({ minHeight, maxHeight }: UseAutoResizeTextareaProps) => {
  ...
  }, [adjustHeight, isMounted]);

  return { textareaRef, adjustHeight, isMounted };
};
...
export const AIInputTextarea = ({
  ...
  const { textareaRef, adjustHeight, isMounted } = useAutoResizeTextarea({
    minHeight,
    maxHeight,
  });
  ...
  return (
    <Textarea
      ...
      onChange={(e) => {
        if (isMounted) adjustHeight();
        onChange?.(e);
      }}
      onKeyDown={handleKeyDown}
      style={isMounted ? {} : { height: `${minHeight}px` }}
      {...props}
    />
  );
};
