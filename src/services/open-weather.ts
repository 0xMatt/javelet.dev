...
export async function getCurrentForecast() {
    return await fetch(`https://api.openweathermap.org/data/2.5/weather?lat=${openWeatherConfig.lat}&lon=${openWeatherConfig.lon}&units=imperial&appid=${openWeatherConfig.key}`);
}